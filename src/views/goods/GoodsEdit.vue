          ...
          <el-form-item v-if="form.specs === 'single'" label="SKU 信息">
            <el-table :data="[singleSku]" size="small" border>
              ...
              <el-table-column label="市场价" width="130">
                <template #default>
                  <el-input-number v-model="singleSku.market_price" :min="0" :precision="2" size="small" controls-position="right" />
                </template>
              </el-table-column>
          ...
          <template v-else>
            ...
            <el-form-item v-if="skuRows.length" label="SKU 明细">
              <el-table :data="skuRows" size="small" border max-height="480">
                ...
                <el-table-column label="市场价" width="130">
                  <template #default="{ row }">
                    <el-input-number v-model="getSku(row.key).market_price" :min="0" :precision="2" size="small" controls-position="right" />
                  </template>
                </el-table-column>
        ...
        <el-form label-width="110px" class="form-block">
          ...
          <el-form-item label="库存扣减">
            <el-radio-group v-model="form.stock">
              <el-radio value="payment">拍下减库存</el-radio>
              <el-radio value="porder">付款减库存</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="支付方式">
            <el-checkbox-group v-model="form.payment_type">
              <el-checkbox value="balance">余额</el-checkbox>
              <el-checkbox value="wechat">微信</el-checkbox>
              <el-checkbox value="alipay">支付宝</el-checkbox>
              <el-checkbox value="union">银联</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-form-item label="售后服务">
            <el-checkbox-group v-model="form.after_sale">
              <el-checkbox value="7day_return">七天退换</el-checkbox>
              <el-checkbox value="broken">坏单包赔</el-checkbox>
              <el-checkbox value="genuine">正品保障</el-checkbox>
              <el-checkbox value="quality">品质保障</el-checkbox>
              <el-checkbox value="after_sale">售后服务</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-form-item label="售后说明">
            <el-input v-model="form.after_sale_content" type="textarea" :rows="3" placeholder="售后说明" />
          </el-form-item>
        </el-form>
      </el-tab-pane>
...
const form = reactive({
  title: '',
  category_ids: [], // 级联路径（取最后一项为 category_id）
  brand_id: undefined,
  image: '',
  images: '',
  description: '',
...
// 单规格 SKU
const singleSku = reactive({ thumbnail: '', price: 0, market_price: 0, stock: 0, sn: '' })

// 多规格：规格组 [{ name, values, draft }]；SKU 明细按组合键存
...
// 获取某组合的 SKU 编辑对象（不存在则初始化）
function getSku(key) {
  if (!skuMap[key]) {
    skuMap[key] = { thumbnail: '', price: 0, market_price: 0, stock: 0, sn: '' }
  }
  return skuMap[key]
}
...
async function loadOptions() {
  ...
  // 本店类目需要 category:list 权限，无权限时静默跳过
...
async function loadDetail() {
  ...
  try {
    const resp = await goodsDetail(goodsId.value)
    const g = resp.goods || {}
    form.title = g.title || ''
    ...
    form.stock = g.stock || 'payment'
    form.payment_type = g.payment_type ? g.payment_type.split(',').filter(Boolean) : []
    form.after_sale = g.after_sale ? g.after_sale.split(',').filter(Boolean) : []
    form.after_sale_content = g.after_sale_content || ''
    ...
    // SKU → 明细（difference 为 JSON 对象字符串）
...
function buildPayload() {
    ...
    skus = [
      {
        ...
        market_price: Number(singleSku.market_price || 0),
  ...
  const goods = {
    ...
    after_sale: form.after_sale.join(','),
    after_sale_content: form.after_sale_content,
...
function goBack() {
  router.push(form.groundingOn || isEdit.value ? '/goods/sale' : '/goods/warehouse')
}
