      ...
      <el-menu :default-active="$route.path" router class="layout-menu">
        <template v-for="item in visibleMenus" :key="item.title">
          <el-sub-menu v-if="item.children" :index="item.title">
            ...
            <el-menu-item v-for="child in item.children" :key="child.path" :index="child.path"><span>{{ child.title }}</span></el-menu-item>
          </el-sub-menu>
          <el-menu-item v-else :index="item.path"><span>{{ item.title }}</span></el-menu-item>
        </template>
      </el-menu>
    </el-aside>
    <el-container>
      ...
      <el-main class="layout-main"><router-view v-if="ready" /></el-main>
    </el-container>
  </el-container>
</template>
...
const menus = [
  { path: '/dashboard', title: '工作台', perm: 'dashboard:view' },
  {
    title: '旅游管理',
    children: [
      { path: '/travel/products', title: '旅游产品', perm: 'travel:product:list' },
      { path: '/travel/inventory', title: '旅游库存', perm: 'travel:inventory:check' },
      { path: '/travel/orders', title: '旅游订单', perm: 'travel:order:list' },
    ],
  },
  {
    title: '数全宝商户中心', children: [
      { path: '/plugin/hwsj/dashboard/index', title: '数据统计' }, { path: '/plugin/hwsj/member/index', title: '会员管理' },
      { path: '/plugin/hwsj/orders/index', title: '订单与对账' }, { path: '/plugin/hwsj/reconciliation/index', title: '对账差异' },
      { path: '/plugin/hwsj/flows/index', title: '资金流水' }, { path: '/plugin/hwsj/messages/index', title: '消息提醒' },
      { path: '/plugin/hwsj/profile/index', title: '商户信息' }, { path: '/plugin/hwsj/recharge/index', title: '预存款充值' },
      { path: '/plugin/hwsj/huifu-enter/index', title: '汇付进件' }, { path: '/plugin/hwsj/withdraw/index', title: '提现管理' },
...
const nickname = computed(() => auth.profile?.staff?.nickname || auth.profile?.merchant?.nickname || '')

onMounted(async () => {
  try {
    await fetchProfile(); ready.value = true
    const perm = router.currentRoute.value.meta?.perm
    if (perm && !hasPerm(perm)) router.replace(firstVisiblePath.value)
  } catch (e) { ElMessage.error(e.message || '加载资料失败') }
})
...
<style scoped>
.layout { height: 100vh; }
.layout-aside { background: #1f2937; overflow-y: auto; }
.layout-main { background: #f3f4f6; overflow-y: auto; }
.logo { color: #fff; font-size: 16px; font-weight: 600; text-align: center; padding: 18px 0; }
.layout-menu { border-right: none; background: transparent; --el-menu-bg-color: transparent; --el-menu-text-color: #d1d5db; --el-menu-hover-bg-color: #374151; --el-menu-active-color: #f59e0b; }
.layout-header { display: flex; align-items: center; justify-content: space-between; background: #fff; border-bottom: 1px solid #e5e7eb; }
