...
$out = 'D:\go_work\simple-admin\merchant-frontend\sql\dbg_freight_logic.txt'
'' | Out-File $out -Encoding UTF8

$hits = Get-ChildItem 'D:\go_work\simple-admin\zzhshopR2-rpc\internal' -Recurse -File -Filter *.go | ForEach-Object {
    $content = Get-Content $_.FullName -Encoding UTF8 -Raw
    if ($content -match 'ShopFreightData|freight|province|citys') {
        $rel = $_.FullName.Substring('D:\go_work\simple-admin\zzhshopR2-rpc\internal'.Length)
